...
#hep.cms.label(llabel="Preliminary", rlabel="CERN 904 Laboratory")

data = pd.read_excel(r'./data/QC3_GE21-MODULE-{}-{}_{}.xlsm'.format(module_type, module_number, qc3_date), engine='openpyxl')
...
#print(t0, t1, time[t0], time[t1], int(t0))
p0 = (26, 0.001)
#popt, pcov = curve_fit(lambda t, a, b: a*np.exp(-b*t), time, pressure)
#popt, pcov = curve_fit(func, time_hr[t0+5:t1-5], pressure[t0+5:t1-5])
#popt, pcov = curve_fit(func, time_hr[t0+7:t1-9], pressure[t0+7:t1-9])
popt, pcov = curve_fit(func, time_hr[t0+8:t1], pressure[t0+8:t1])
a, b = popt
ymin = pressure[t1]-2.0
ymax = pressure[t0]+2.0
plt.ylim([ymin, ymax])
#plt.plot(time_hr[:t1+1], pressure[:t1+1], 'ok', markersize=2, label='Data')
plt.plot(time_hr[:t1+1], pressure[:t1+1], 'ok', markersize=2, label='GE21-MODULE-{}-{}'.format(module_type, module_number))
...
#    ' p0                %.2f ' % a,
#    ' p1                %.2f ' % b))
#props = dict(boxstyle='square, pad=0.5', facecolor='none', alpha=1., edgecolor='k')
#plt.text(0.645, ymax-(ymax-ymin)/3.7, 'p0              %.2f' % a, fontsize = 20, color='b')
#plt.text(0.645, ymax-(ymax-ymin)/3.7-(ymax-ymin)/16, 'p1              %.2f' % b, fontsize = 20, color='b')
#plt.text(0.645, ymax-(ymax-ymin)/3.7-(ymax-ymin)*30/(16*14), r' $\tau$             %.2f hr' % t, fontsize=22, fontweight='bold', color='b')
plt.text(0.53, ymax-(ymax-ymin)/3.7, 'p0              %.2f mbar' % a, fontsize = 20, color='b')
#plt.text(0.6, ymax-(ymax-ymin)/3.7-(ymax-ymin)/16, 'p1              %.2f' % b, fontsize = 20, color='b')
plt.text(0.53, ymax-(ymax-ymin)/3.7-(ymax-ymin)/16, r'$\tau$                %.2f h' % t, fontsize = 20, color='b', fontweight='bold')
#plt.text(0.6, ymax-(ymax-ymin)/3.7-(ymax-ymin)*30/(16*14), r' $\tau$               %.2f hr' % t, fontsize=20, fontweight='bold', color='b')
plt.text(0, 21.4, 'GE2/1 Module Production', fontsize=20) 
plt.text(0, 20.7, 'Gas = $CO_{2}$', fontsize=20) 
...
#plt.savefig('QC3_{}-{}_test.png'.format(module_type, module_number))
plt.clf()
...
#plt.text(500, 1.31, 'GE2/1 Module Production', fontsize=20) #, fontproperties=font)
#plt.text(500, 0.94, 'Gas = $CO_{2}$', fontsize=20) #, fontproperties=font)
#plt.text(500, 0.57, '$R_{n}$ = 5.0 $M\Omega$' % r_m, fontsize=20) #, fontproperties=font)
#plt.text(500, 0.2, '$R_{m}$ = %.3f $M\Omega$' % r_m, fontsize=20) #, fontproperties=font)
lg = font_manager.FontProperties(#weight='bold',
                                 style='normal', size=20)
plt.xticks([200, 400, 600, 800, 1000])
plt.xlabel('Divider Current $I_{divider} \ [\mu$A]')
plt.ylabel('Applied Voltage V [kV]')
#plt.ylabel('Applied Voltage V (kV)', horizontalalignment='left')
plt.legend(loc='lower right', prop=lg)
#plt.legend(loc='upper left', prop=lg)
#plt.show()
